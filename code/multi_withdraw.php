...
if($_SERVER["REQUEST_METHOD"] == "POST" && $_SESSION["username"] != ""){
   ...
   //      $usr_id = trim($_POST["usr_id"]);
		...
		if ($status == '2') {
			...
			$qry = "UPDATE `transaction` set  contestid='$status' where userid='$value' and date1='$date1' ";
			...
			$qry0 = "INSERT INTO `transaction` (`userid`, `amount`, `date1`, `type`) VALUES ('$value','$amount','$date1','cancelled')";
			$res0 = mysqli_query($conn,$qry0);
			$qry1 = "UPDATE `API_user_account` set  usr_paytm_isVarified='3',usr_bank_isVarified='3',usr_act_remark=''  where usr_id='$value' ";
			$res1 = mysqli_query($conn,$qry1);
			$qry2 = "UPDATE `wallet1` set  wamount=wamount+'$amount' where userid='$value' ";
