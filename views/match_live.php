        ...
        <div class="col-xs-12">
          ...
          <div class="box">
            <div class="box-header">
              <h3 class="box-title">Live Matches</h3>
            </div>
            <!-- /.box-header -->
			<?php
			require_once "../db/config.php";
			$sql = "SELECT * FROM API_matches WHERE `mat_status` = '3' ORDER BY `mat_startdate` DESC";
            ...
            <div class="box-body">
              <table id="example1" class="table table-bordered table-striped">
				...
				while($rows = mysqli_fetch_array($result))
				{
					...
					$mat_id3 = $rows['mat_id'];
					$sql3 = "SELECT sqd_ply11_ids FROM API_match_squad WHERE `sqd_mat_id` = '$mat_id3'";
					$result3 = mysqli_query($conn,$sql3);
					$num3 = mysqli_num_rows($result3);
					$row3 = mysqli_fetch_assoc($result3);
					?>
					<tr>
					  <td><?php echo $i;?></br>(<?php echo $rows['mat_id'];?>)</td>
					  ...
					  <td>
			  			<a href="https://matchonsports.com/app_code/FantasyController/contestAutoCancel?match_id=<?php echo $rows['mat_id'];?>" target="_blank">
					  	<button type="button" class="btn btn-block btn-danger">Contest Auto Cancel </button>
					  </a>


					  	<a href="https://matchonsports.com/app_code/FantasyController/getLiveScoreFromAdmin?match_id=<?php echo $rows['mat_id'];?>" target="_blank">
					  	<button type="button" class="btn btn-block btn-primary">Update Points </button>
					  </a>


					  	<a href="https://matchonsports.com/app_code/FantasyController/updateUserRankAndPrice?match_id=<?php echo $rows['mat_id'];?>" target="_blank">
					  	<button type="button" class="btn btn-block btn-success">Update Rank </button>
					  </a>

					  	<?php
					  	if ($row3['sqd_ply11_ids'] == '') {
						  	?>
						  ...
						  }else if($rows['mat_isVisible'] == '0')
						  {
							  ...
							  <input type="hidden" name="link" value="match_live">
							  <button type="submit" class="btn btn-block btn-danger">Inactive</button>
							  </form>
							  <?php
						  }
						  ?>
						  <form action="../code/status_change" method="post">
						  <input type="hidden" name="table_name" value="API_matches">
						  <input type="hidden" name="fld_id" value="mat_id">
						  <input type="hidden" name="mat_id" value="<?php echo $rows['mat_id'];?>">
						  <input type="hidden" name="fld_name" value="mat_status">
						  <input type="hidden" name="mat_status" value="4">
						  <input type="hidden" name="link" value="match_live">
						  <button type="submit" class="btn btn-block btn-danger">Cancel Match</button>
						  </form>
